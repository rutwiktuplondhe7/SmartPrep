  ...
  return (
    <div className="relative flex items-center justify-center">
      ...
      <div className="absolute flex flex-col items-center justify-center">
        <span className="text-4xl font-bold text-white">{safeValue}%</span>
...
const Summary = () => {
  ...
  useEffect(() => {
    const fetchSummary = async () => {
      const res = await axiosInstance.get(`/api/interview/${sessionId}/summary`);
      setData(res.data);
    };
    fetchSummary();
  }, [sessionId]);
  ...
  const avgPauseRatio =
    answers.length > 0
      ? (answers.reduce((sum, a) => sum + (a.pauseRatio || 0), 0) / answers.length).toFixed(2)
      : "0.00";

  const avgSpeakingRate =
    answers.length > 0
      ? Math.round(answers.reduce((sum, a) => sum + (a.speakingRate || 0), 0) / answers.length)
      : 0;

  const avgEnergyVariance =
    answers.length > 0
      ? (answers.reduce((sum, a) => sum + (a.rmsVariance || 0), 0) / answers.length).toFixed(3)
      : "0.000";

  const avgFillerCount =
    answers.length > 0
      ? Math.round(answers.reduce((sum, a) => sum + (a.fillerCount || 0), 0) / answers.length)
      : 0;
    ...
    <div className="min-h-screen relative overflow-hidden bg-[#0f172a] text-white px-6 py-16">
      <div className="relative max-w-6xl mx-auto">
        ...
        <div className="mb-16">
          ...
          <div className="h-1 w-24 bg-gradient-to-r from-indigo-400 to-cyan-400 mt-4 rounded-full" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-20">
          ...
          {/* Completion */}
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-10 rounded-3xl shadow-xl text-center flex flex-col items-center justify-center">
            <p className="text-gray-400 mb-4">Completion Rate</p>
            <p className="text-5xl font-bold">
              {totalAnswered}
              <span className="text-gray-400 text-2xl font-medium"> / {totalQuestions}</span>
            </p>
          </div>

          {/* Confidence — fused audio + video */}
          ...
          {/* Clarity — audio only, hover shows breakdown */}
          <div className="relative group bg-white/5 backdrop-blur-xl border border-white/10 p-10 rounded-3xl shadow-xl flex items-center justify-center overflow-hidden">
            ...
            <div className="hidden group-hover:flex absolute inset-0 p-8 flex-col justify-center items-center text-center bg-[#0f172a] transition-all duration-300">
              <h3 className="text-xl font-semibold text-blue-400 mb-4">Clarity Analytics</h3>
              <p className="text-sm text-gray-400">
                Avg Pause Ratio:{" "}
                <span className="text-white font-semibold">{avgPauseRatio}</span>
              </p>
              <p className="text-sm text-gray-400 mt-2">
                Avg Speaking Rate:{" "}
                <span className="text-white font-semibold">{avgSpeakingRate} WPM</span>
              </p>
              <p className="text-sm text-gray-400 mt-2">
                Energy Stability:{" "}
                <span className="text-white font-semibold">{avgEnergyVariance}</span>
              </p>
              <p className="text-sm text-gray-400 mt-2">
                Avg Fillers Used:{" "}
                <span className="text-white font-semibold">{avgFillerCount}</span>
              </p>
            </div>
          </div>
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-12 rounded-3xl shadow-xl">
          <h2 className="text-3xl font-semibold mb-12">Detailed Breakdown</h2>

          {answers.map((ans, index) => (
            <div key={index} className="mb-12 pb-10 border-b border-white/10">
              ...
              <div className="bg-black/30 border border-white/10 p-6 rounded-2xl mb-5">
                ...
                <p className="text-gray-200">{ans.transcript}</p>
              </div>
